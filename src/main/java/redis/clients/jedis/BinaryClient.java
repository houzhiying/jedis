...
public class BinaryClient extends Connection {
  ...
  public void hsetex(final byte[] key, final long expire, final byte[] field, final byte[] value) {
    sendCommand(HSETEX, key, toByteArray(expire), field, value);
  }

  public void hgetex(final byte[] key, final long expire, final byte[] field) {
    sendCommand(HGETEX, key, toByteArray(expire), field);
  }

  public void hdelex(final byte[] key, final long expire, final byte[]... fields) {
    final byte[][] params = new byte[fields.length + 2][];
    params[0] = key;
    params[1] = toByteArray(expire);
    System.arraycopy(fields, 0, params, 2, fields.length);
    sendCommand(HDELEX, params);
  }
  ...
  public void hclear(final byte[] key) {
    sendCommand(HCLEAR, key);
  }

  public void hexpire(final byte[] key, final int seconds) {
    sendCommand(HEXPIRE, key, toByteArray(seconds));
  }

  public void httl(final byte[] key) {
    sendCommand(HTTL, key);
  }

  public void hpersist(final byte[] key) {
    sendCommand(HPERSIST, key);
  }
  ...
  public void lclear(final byte[] key) {
    sendCommand(LCLEAR, key);
  }

  public void lexpire(final byte[] key, final int seconds) {
    sendCommand(LEXPIRE, key, toByteArray(seconds));
  }

  public void lttl(final byte[] key) {
    sendCommand(LTTL, key);
  }

  public void lpersist(final byte[] key) {
    sendCommand(LPERSIST, key);
  }
  ...
  public void sclear(final byte[] key) {
    sendCommand(SCLEAR, key);
  }

  public void smclear(final byte[]... key) {
    sendCommand(SMCLEAR, key);
  }

  public void sexpire(final byte[] key, final int seconds) {
    sendCommand(SEXPIRE, key, toByteArray(seconds));
  }

  public void sttl(final byte[] key) {
    sendCommand(STTL, key);
  }

  public void spersist(final byte[] key) {
    sendCommand(SPERSIST, key);
  }
  ...
  public void zclear(final byte[] key) {
    sendCommand(ZCLEAR, key);
  }

  public void zexpire(final byte[] key, final int seconds) {
    sendCommand(ZEXPIRE, key, toByteArray(seconds));
  }

  public void zttl(final byte[] key) {
    sendCommand(ZTTL, key);
  }

  public void zpersist(final byte[] key) {
    sendCommand(ZPERSIST, key);
  }
  ...
  public void zrangeByScoreWithScores(final byte[] key, final double min, final double max,
    ...
    sendCommand(ZRANGEBYSCORE, key, toByteArray(min), toByteArray(max), WITHSCORES.raw, LIMIT.raw,
        toByteArray(offset),
        toByteArray(count));
  }
  ...
  public void zrangeByScoreWithScores(final byte[] key, final String min, final String max,
      final int offset, final int count) {
    sendCommand(ZRANGEBYSCORE, key, min.getBytes(), max.getBytes(), WITHSCORES.raw, LIMIT.raw,
        toByteArray(offset),
        toByteArray(count));
  }
  ...
  public void zrevrangeByScoreWithScores(final byte[] key, final String max, final String min,
    ...
    sendCommand(ZREVRANGEBYSCORE, key, max.getBytes(), min.getBytes(), WITHSCORES.raw, LIMIT.raw,
        toByteArray(offset), toByteArray(count));
  }
  ...
  public void zrangeByScoreWithScores(final byte[] key, final byte[] min, final byte[] max,
      final int offset, final int count) {
    sendCommand(ZRANGEBYSCORE, key, min, max, WITHSCORES.raw, LIMIT.raw, toByteArray(offset),
        toByteArray(count));
  }

  public void zrevrangeByScoreWithScores(final byte[] key, final byte[] max, final byte[] min,
      final int offset, final int count) {
    sendCommand(ZREVRANGEBYSCORE, key, max, min, WITHSCORES.raw, LIMIT.raw, toByteArray(offset),
        toByteArray(count));
  }
  ...
  public void georadius(byte[] key, double longitude, double latitude, double radius,
      GeoUnit unit) {
  ...
  public void jset(final byte[] key, final byte[] path, final byte[] value) {
    sendCommand(JsonCommand.SET, key, path, value);
  }

  public void jget(final byte[] key, final byte[] path) {
    sendCommand(JsonCommand.GET, key, path);
  }

  public void jexists(final byte[]... key) {
    sendCommand(JsonCommand.EXISTS, key);
  }

  public void jdel(final byte[] key, final byte[] path) {
    sendCommand(JsonCommand.DEL, key, path);
  }

  public void jmkget(final byte[]... key) {
    sendCommand(JsonCommand.MKGET, key);
  }

  public void jtype(final byte[] key, final byte[] path) {
    sendCommand(JsonCommand.TYPE, key, path);
  }

  public void jobjlen(final byte[] key) {
    sendCommand(JsonCommand.OBJLEN, key);
  }

  public void jarrappend(final byte[] key, final byte[] path, final byte[] value) {
    sendCommand(JsonCommand.ARRAPPEND, key, path, value);
  }

  public void jarrlen(final byte[] key) {
    sendCommand(JsonCommand.ARRLEN, key);
  }

  public void jarrpop(final byte[] key) {
    sendCommand(JsonCommand.ARRPOP, key);
  }
