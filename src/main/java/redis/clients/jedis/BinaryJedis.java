...
public class BinaryJedis implements BasicCommands, BinaryJedisCommands, MultiKeyBinaryCommands,
  ...
  public BinaryJedis(final String host) {
    ...
    if (uri.getScheme() != null && (uri.getScheme().equals("redis") || uri.getScheme()
        .equals("rediss"))) {
   ...
   * on the specified key. After the timeout the key will be automatically deleted by the server. A
   * key with an associated timeout is said to be volatile in Redis terminology. <p> Voltile keys
   * are stored on disk like the other keys, the timeout is persistent too like all the other
   * aspects of the dataset. Saving a dataset containing expires and stopping the server does not
   * stop the flow of time as Redis stores on disk the time when the key will no longer be available
   * as Unix time, and not the remaining milliseconds. <p> Since Redis 2.1.3 you can update the
   * value of the timeout of a key already having an expire set. It is also possible to undo the
   * expire at all turning the key into a normal key using the {@link #persist(byte[]) PERSIST}
   ...
   * EXPIRE. If the Key does not exists or does not have an associated expire, -1 is returned.
   */
  ...
  public String hsetex(final byte[] key, final long expire, final byte[] field, final byte[] value) {
    checkIsInMultiOrPipeline();
    client.hsetex(key, expire, field, value);
    return client.getStatusCodeReply();
  }

  public String hmsetex(final byte[] key, final long expire, final Map<byte[], byte[]> hash) {
    checkIsInMultiOrPipeline();
    client.hmsetex(key, expire, hash);
    return client.getStatusCodeReply();
  }

  public byte[] hgetex(final byte[] key, final long expire, final byte[] field) {
    checkIsInMultiOrPipeline();
    client.hgetex(key, expire, field);
    return client.getBinaryBulkReply();
  }

  public Long hdelex(final byte[] key, final long expire, final byte[] field) {
    checkIsInMultiOrPipeline();
